...
int *allocate_int_array(const size_t n)
{
...
void randomly_fill_uniform_matrix(float **A, const size_t A_rows, const size_t A_cols)
{
   for (size_t i=0; i< A_rows; i++) {
     for (size_t j=0; j< A_cols; j++) {
...
void randomly_fill_positive_int_matrix(int **A, const size_t A_rows, const size_t A_cols, const unsigned int max_value)
{
   for (size_t i=0; i< A_rows; i++) {
     for (size_t j=0; j< A_cols; j++) {
   	   A[i][j]=(int)(rand()%(max_value));
     }
   }
}

int same_int_matrix(int **A, int **B, const size_t rows, const size_t cols)
{
   for (size_t i=0; i<rows; i++) {
     for (size_t j=0; j<cols; j++) {
        if (A[i][j] != B[i][j]) {
